...
class IMDBDataParser:
    ...
    def parse_file(self, filename):
        ...
        with open(filename) as m_page:
            ...
            d['rating'] = float(main_top.find('span', {'itemprop': 'ratingValue'}).text)
            ...
            writers = credit_summary_items[1].find_all('span', {'itemprop': 'name'})
            d['writer'] = [w.text for w in writers]

            main_bottom = content.select('div#main_bottom')[0]

            d['imdb_rating'] = int(re.search('\d+', main_bottom.find('a', {'href': '/chart/top?ref_=tt_awd'}).text).group(0))
